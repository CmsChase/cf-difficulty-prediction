...
from cf_diff import baselines, features
...
def test_split_config_fingerprint_mismatch_is_rejected() -> None:
    """Training cannot combine a split generated by another config."""
    split = _split_assignment("contest_grouped")
    split["config_fingerprint_sha256"] = "a" * 64
    with pytest.raises(baselines.BaselineError, match="does not match"):
        baselines.validate_split_config_fingerprint(
            split,
            "b" * 64,
            label="contest split",
        )
...
def test_run_baselines_writes_outputs(tmp_path: Path) -> None:
    ...
    fingerprint = features.experiment_config_fingerprint(
        features.load_experiment_config(config_path)
    )
    feature_columns_path.write_text(
        json.dumps(
            {
    ...
    for path in (contest_split_path, time_split_path):
        split = pd.read_parquet(path)
        split["config_fingerprint_sha256"] = fingerprint
        split.to_parquet(path, engine="pyarrow", index=False)
    ...
    assert set(metrics["config_fingerprint_sha256"]) == {fingerprint}
