...
def load_baseline_config(path: Path = DEFAULT_CONFIG_PATH) -> BaselineConfig:
    """Load the seed through the same strict experiment config parser."""
    try:
        experiment = load_experiment_config(path)
    except (FeatureError, json.JSONDecodeError, OSError, ValueError) as error:
        raise BaselineError(f"Invalid experiment config: {error}") from error
    return BaselineConfig(
        random_seed=experiment.random_seed,
        config_fingerprint_sha256=experiment_config_fingerprint(experiment),
    )
...
def load_feature_columns(
    path: Path,
    *,
    expected_config_fingerprint_sha256: str | None = None,
) -> list[str]:
    ...
    if expected_config_fingerprint_sha256 is not None:
        actual = payload.get("config_fingerprint_sha256")
        if actual != expected_config_fingerprint_sha256:
            raise BaselineError(
                "Feature metadata config fingerprint does not match the "
                "requested experiment config."
            )
    return list(raw_columns)


def validate_split_config_fingerprint(
    split_assignment: pd.DataFrame,
    expected_config_fingerprint_sha256: str,
    *,
    label: str,
) -> None:
    """Require split assignments generated by the same effective config."""
    column = "config_fingerprint_sha256"
    if column not in split_assignment.columns:
        raise BaselineError(f"{label} lacks {column} provenance.")
    values = set(split_assignment[column].dropna().astype(str))
    if values != {expected_config_fingerprint_sha256}:
        raise BaselineError(
            f"{label} config fingerprint does not match the requested config."
        )
...
def run_baselines(
    *,
    ...
    try:
        ...
        validate_split_config_fingerprint(
            contest_split,
            config.config_fingerprint_sha256,
            label="Contest-grouped split",
        )
        validate_split_config_fingerprint(
            time_split,
            config.config_fingerprint_sha256,
            label="Forward-time split",
        )
        ...
        for frame in (
            ...
            frame["config_fingerprint_sha256"] = (
                config.config_fingerprint_sha256
            )
